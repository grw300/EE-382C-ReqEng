...
\node [proc, densely dotted, it] (p0) {New Semester};
% Use join to connect a node to the previous one 
\node [term, join]      {Course Info};
\node [proc, join] (p1) {Get Course Data};
\node [proc, join]      {Create Survey};
\node [proc, join]      {Dispatch Survey};
\node [test, join] (t1) {Got msg?};

\node [term, right=of p1] (t2) {UTEID Hash};
\node [proc, join] {Student Name};
...
\node [test, join] (t3) {Got msg?};
\node [test] (t4) {Capacity?};
\node [test] (t5) {$k \mathbin{{-}{=}} 1$};
% We position the next block explicitly as the first block in the
% second column.  The chain 'comes along with us'. The distance
% between columns has already been defined, so we don't need to
% specify it.
%\node [proc, fill=lcfree!25, right=of p0] (p4) {Reset congestion};
...
\node [above=0mm of t2, it] {(FERPA Concerns)};
